...
RECON_DIR = "r2/experiments/temporal-writing/c-hybrid/round-10/reconciled"
...
def title_and_teaser(prose: str, number: int) -> tuple[str, str]:
    ...
    teaser = next((line.strip() for line in lines[1:] if line.strip() and line.strip() != "---"), "")
...
for number in range(START, END + 1):
    ...
    manifest = {
        ...
        "images": [],
        "navigation": {"previous": f"r2-ch{number - 1:03d}", "next": None if number == END else f"r2-ch{number + 1:03d}"},
    }
    (R2 / f"data/chapters/ch{number:03d}.json").write_text(json.dumps(manifest, separators=(",", ":")) + "\n", encoding="utf-8")
...
for number in range(START, END + 1):
    ...
    registry["chapters"][f"r2-ch{number:03d}"] = {
        ...
        "title": title,
        "status": "in_progress",
        "pipeline": {"scene_packets": "in_progress", "audio": "not_started", "written": "published", "story_sync": "in_progress", "image_plan": "not_started", "image_generation": "not_started", "image_review": "not_started", "image_integration": "not_started"},
...
replacements = {
    "range(1, 305)": "range(1, 331)",
    "'r2-ch304'": "'r2-ch330'",
    "test_written_frontier_is_public_through_chapter_three_hundred_four": "test_written_frontier_is_public_through_chapter_three_hundred_thirty",
    "None if number == 304": "None if number == 330",
}
...
assert json.loads(project_path.read_text(encoding="utf-8"))["current_chapter"] == "r2-ch330"
assert json.loads(registry_path.read_text(encoding="utf-8"))["current_chapter"] == "r2-ch330"
for number in range(1, END + 1):
    assert (R2 / f"data/chapters/ch{number:03d}.json").exists(), number
print("built R2 written publication through Chapter 330")
