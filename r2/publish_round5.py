...
SOURCE_DIR = 'r2/experiments/temporal-writing/c-hybrid/round-05/reconciled'
START = 191
END = 205
OLD_END = 190
...
test = test.replace('range(1, 191)', 'range(1, 206)')
test = test.replace("'r2-ch190'", "'r2-ch205'")
test = test.replace('chapter_one_hundred_ninety', 'chapter_two_hundred_five')
test = test.replace('number == 190', 'number == 205')
test_path.write_text(test, encoding='utf-8')

project = json.loads(project_path.read_text(encoding='utf-8'))
assert project['current_chapter'] == 'r2-ch205'
assert project['chapters'][-1] == 'r2-ch205'
for n in range(START, END + 1):
...
print('Round 05 public reader state 191-205 built from reconciled experiment authority: OK')
