...
SOURCE_DIR = 'r2/experiments/temporal-writing/c-hybrid/round-06/reconciled'
START = 206
END = 226
OLD_END = 205
...
test = test.replace('range(1, 206)', 'range(1, 227)')
test = test.replace("'r2-ch205'", "'r2-ch226'")
test = test.replace('chapter_two_hundred_five', 'chapter_two_hundred_twenty_six')
test = test.replace('number == 205', 'number == 226')
test_path.write_text(test, encoding='utf-8')

project = json.loads(project_path.read_text(encoding='utf-8'))
assert project['current_chapter'] == 'r2-ch226'
assert project['chapters'][-1] == 'r2-ch226'
for n in range(START, END + 1):
...
print('Round 06 public reader state 206-226 built from reconciled experiment authority: OK')
