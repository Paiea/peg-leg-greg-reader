...
SOURCE_DIR = 'r2/experiments/temporal-writing/c-hybrid/round-07/reconciled'
START = 227
END = 252
OLD_END = 226
...
assert old['written']['status'] == 'published'
assert old['navigation']['next'] is None
...
assert project['current_chapter'] == f'r2-ch{OLD_END:03d}'
...
assert registry['current_chapter'] == f'r2-ch{OLD_END:03d}'
...
assert 'range(1, 227)' in test
assert "'r2-ch226'" in test
assert 'chapter_two_hundred_twenty_six' in test
assert 'number == 226' in test
test = test.replace('range(1, 227)', 'range(1, 253)')
test = test.replace("'r2-ch226'", "'r2-ch252'")
test = test.replace('chapter_two_hundred_twenty_six', 'chapter_two_hundred_fifty_two')
test = test.replace('number == 226', 'number == 252')
test_path.write_text(test, encoding='utf-8')

project = json.loads(project_path.read_text(encoding='utf-8'))
assert project['current_chapter'] == 'r2-ch252'
assert project['chapters'][-1] == 'r2-ch252'
assert len(project['chapters']) == 252
for n in range(START, END + 1):
...
print('Round 07 public reader state 227-252 built from reconciled experiment authority: OK')
