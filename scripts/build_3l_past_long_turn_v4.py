#!/usr/bin/env python3
"""Assemble Record 001 V4 from speaker-pure Deep/Fancy captures and explicit pauses."""
...
def main() -> None:
    ...
    ap.add_argument("--manifest", type=Path, default=Path("3l/audio/experiments/record-001-v4-captures.json"))
    ap.add_argument("--capture-dir", type=Path, required=True)
    ap.add_argument("--output", type=Path, default=Path("3l/assets/audio/record-001-past-long-turn-v4.mp3"))
    ap.add_argument("--receipt", type=Path, default=Path("3l/audio/verification/record-001-past-long-turn-v4.json"))
    a = ap.parse_args()
    ...
    assert manifest["experiment"] == "past-tense-long-turn-dragon-v4"
    assert design["narration_tense"] == "past"
    assert design["dramatic_block_count"] == 17
    assert design["dragon_turn_count"] == 8
    assert len({row[1] for row in clips}) == 17
    assert len({row[1] for row in clips if row[2] == "dragon"}) == 8
    ...
    with tempfile.TemporaryDirectory(prefix="3l-past-v4-") as td:
        temp = Path(td)
