...
def manuscript_body(path: Path) -> str:
    ...
    for line in lines:
        ...
        body.append(line)
    if headings < 2:
        raise ValueError(f"{path}: expected two headings before prose")
    return "\n".join(body).strip()


def record_body_html(path: Path) -> str:
    body = manuscript_body(path)
    paragraphs = [paragraph.strip() for paragraph in body.split("\n\n") if paragraph.strip()]
    if not paragraphs:
        raise ValueError(f"{path}: record prose is empty")
...
def build() -> list[tuple[str, str, bool, str | None]]:
    ...
    manuscript_by_record = {record: path for path, (record, _) in zip(manuscripts, metadata)}
    ...
    for record, title, has_audio, filename in records:
        prose_html = record_body_html(manuscript_by_record[record])
        (RECORD_DIR / f"{record}.html").write_text(
            record_page(record, title, prose_html, has_audio, filename, first, last), encoding="utf-8"
        )
