...
def _art_figure(path: Path, number: int) -> str:
    src = "../" + path.as_posix()
    return '<figure class="chapter-art scene-illustration">' + f'<img src="{html.escape(src, quote=True)}" alt="A story scene from Chapter {number}." loading="lazy"/>' + "</figure>"
...
def main() -> int:
    ...
    for number in wanted:
        ...
        (CHAPTERS_DIR / f"{number:03d}.html").write_text(render_chapter(chapter, all_numbers, chapter_art(number)), encoding="utf-8")
