...
def href_for(number: int, generated: set[int], *, from_chapter: bool) -> str:
    if number in generated:
        return f'{number:03d}.html' if from_chapter else f'{number:03d}.html'
    return f'../light.html?chapter={number}' if from_chapter else f'../light.html?chapter={number}'
...
def range_groups(all_chapters: dict[int, Chapter]) -> list[tuple[str, list[int]]]:
    ...
    if recovered: groups.append(('Chapters 156–219', recovered))
    if upper_published: groups.append(('Chapters 100–155', upper_published))
    if mid_published: groups.append(('Chapters 83–99', mid_published))
    if early: groups.append(('Chapters 1–82', early))
    return groups
...
def selected_numbers(spec: str, all_chapters: dict[int, Chapter]) -> list[int]:
    ...
    if start > end:
        start, end = end, start
    return [n for n in range(start, end + 1) if n in all_chapters]
...
def main() -> int:
    ...
    generated = {int(item['number']) for item in previous.get('chapters', []) if str(item.get('number', '')).isdigit()}
    ...
    manifest_chapters = [
        ...
        for n in sorted(generated) if n in all_chapters
    ]
    latest = max(all_chapters) if all_chapters else None
