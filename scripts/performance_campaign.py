...
def build_worker_prompt(packet: dict[str, Any], scene_record: dict[str, Any]) -> str:
    ...
    try:
        ...
        view = {
            ...
            "source": {"hash": packet.get("source_hash")},
        }
    payload = {
        "packet": packet,
        "view": view,
    ...
    return (
        ...
        "or mutate canon. Prefer deterministic facts and the supplied compiler view. Return one JSON object only. "
...
def _default_codex_executor(
    ...
    override = os.environ.get("PLG_CODEX_EXEC")
    if override:
        command = shlex.split(override)
    else:
        command = ["codex", "exec", "--ephemeral", "--sandbox", "read-only", "--color", "never"]
        if config.get("model"):
            command.extend(["--model", str(config["model"])])
        if config.get("reasoning_tier"):
            command.extend(["-c", f"model_reasoning_effort={config['reasoning_tier']}"])
        command.append("-")
    ...
    try:
        ...
        raise RuntimeError("Codex worker final message was not one JSON object") from exc
...
def _load_scene_for_packet(compiled_root: Path, packet: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        return {
            "scene_id": packet.get("scene_id"),
            "source": {"hash": packet.get("source_hash"), "paragraphs": []},
            "mechanical": {},
            "dependencies": {},
        }
...
def _persist_worker_result(
    ...
    if isinstance(comparison, dict):
        scene = funnel.set_comparison_result(
            scene,
            comparison,
            compiler=compiler_name,
            dependency_hash=dependency_hash,
    ...
    if match:
        _write_json(
            compiled_root / match.group("chapter") / f"s{int(match.group('number')):03d}.json",
            scene,
        )
...
def run_campaign(
    ...
    *,
    ...
    completed_ids = {
        ...
        if row.get("status") == "completed" and isinstance(row.get("packet_id"), str)
    }
    ...
    def execute_one(packet: dict[str, Any]) -> tuple[str, dict[str, Any]]:
        ...
        for attempt in range(max_retries + 1):
            ...
            _append_jsonl(
                telemetry_path,
                {
                    ...
                    "reasoning_tier": execution.get("reasoning_tier"),
                    "attempt": attempt + 1,
                },
            )
        ...
        return "failed", {
            ...
            "scene_id": packet["scene_id"],
            "status": "failed",
            "error": str(last_error) if last_error else "worker failed",
    ...
    _write_jsonl(
        campaign_root / "results.jsonl",
        existing_results + sorted(new_results, key=lambda row: str(row.get("packet_id", ""))),
    )
    existing_failures = _read_jsonl(campaign_root / "failures.jsonl")
    _write_jsonl(
        campaign_root / "failures.jsonl",
        existing_failures + sorted(failures, key=lambda row: str(row.get("packet_id", ""))),
    )
    reduced = reduce_campaign(campaign_root)
    return {
        "campaign_id": campaign.get("campaign_id"),
        "workers_launched": len(pending),
...
def _token_sum(completes: list[dict[str, Any]], field: str) -> int | None:
...
def _telemetry_metrics(
    ...
    *,
    ...
    events = _read_jsonl(campaign_root / "telemetry.jsonl")
    ...
    rejections = [event for event in events if event.get("event") == "integration_rejected"]
    campaign = _read_json(campaign_root / "campaign.json") if (campaign_root / "campaign.json").exists() else {}
    plan = campaign.get("plan") if isinstance(campaign.get("plan"), dict) else {}
    scenes_considered = int(plan.get("scenes_considered", 0) or 0)
    cache_hits = int(plan.get("cache_hits", 0) or 0)
    reviewed = source_wins + candidates + ambiguous
    false_positive_labels = sum(1 for row in results if row.get("review_label") == "false_positive")
    labeled_reviews = sum(1 for row in results if isinstance(row.get("review_label"), str))
    return {
        "scenes_considered": scenes_considered,
        "cache_hits": cache_hits,
        "cache_hit_rate": _rate(cache_hits, scenes_considered),
        "worker_launch_events": len(launches),
        "retry_events": len(retries),
        "failure_rate": _rate(failures, max(len(completes) + failures, 0)),
        "source_win_rate": _rate(source_wins, reviewed),
        "candidate_rate": _rate(candidates, reviewed),
        "ambiguous_rate": _rate(ambiguous, reviewed),
        "survivor_rate": _rate(survivors, reviewed),
        "useful_survivor_per_worker": _rate(survivors, len(launches)),
        "false_positives": false_positive_labels if labeled_reviews else None,
        "false_positive_rate": _rate(false_positive_labels, labeled_reviews) if labeled_reviews else None,
        "integration_rejections": len(rejections),
        "elapsed_worker_ms": sum(int(event.get("elapsed_ms", 0) or 0) for event in completes),
        "reported_input_tokens": _token_sum(completes, "input_tokens"),
        "reported_output_tokens": _token_sum(completes, "output_tokens"),
        "reported_cached_tokens": _token_sum(completes, "cached_tokens"),
    }
...
def get_campaign_result(campaign_root: Path) -> dict[str, Any]:
    ...
    return _read_json(summary_path) if summary_path.exists() else reduce_campaign(campaign_root)
...
def _patch_interval(paragraphs: list[str], patch: dict[str, Any]) -> tuple[int, int]:
    start = _normalized_anchor(str(patch["start"]))
    end = _normalized_anchor(str(patch["end"]))
    starts = [index for index, value in enumerate(paragraphs) if _normalized_anchor(value) == start]
    if len(starts) != 1:
        raise ValueError(f"start boundary matched {len(starts)} times during campaign preflight")
    start_index = starts[0]
    ends = [
        index
        for index, value in enumerate(paragraphs)
        if index >= start_index and _normalized_anchor(value) == end
    ]
    if len(ends) != 1:
        raise ValueError(f"end boundary matched {len(ends)} times during campaign preflight")
    return start_index, ends[0]


def _preflight_survivor_patches(
    chapter_root: Path,
    ...
    for chapter, records in sorted(by_chapter.items()):
        ...
        if not path.exists():
            raise ValueError(f"canonical chapter missing: {path.name}")
        paragraphs = funnel.extract_paragraphs(path.read_text(encoding="utf-8"))
        intervals: list[tuple[int, int]] = []
        for record in records:
            for patch in record["patches"]:
                intervals.append(_patch_interval(paragraphs, patch))
        intervals.sort()
        for previous, current in zip(intervals, intervals[1:]):
            if current[0] <= previous[1]:
                raise ValueError(
                    f"overlapping survivor patches in chapter {chapter:03d}: {previous} overlaps {current}"
                )
    ...
    *,
    ...
    if current_authority != source_authority:
        _append_jsonl(
            telemetry_path,
            {
                "event": "integration_rejected",
                "reason": "authority_drift",
                "campaign_authority": source_authority,
    ...
    try:
        _preflight_survivor_patches(chapter_root, by_chapter)
    except ValueError as exc:
        _append_jsonl(
            telemetry_path,
            {"event": "integration_rejected", "reason": "patch_preflight", "error": str(exc)},
        )
        raise
    ...
    changed = funnel.apply_batch_to_root(batch, chapter_root)
    _append_jsonl(
        telemetry_path,
        {
