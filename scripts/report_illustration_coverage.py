...
def render_coverage_report(summary: dict[str, int | list[int]]) -> str:
    return "\n".join(
        [
            ...
            "## Production queue",
            "",
            f"- Candidate, needs prompt pack: {summary.get('candidate_unbriefed', 0)}",
            f"- Prompt ready / generation ready: {summary.get('prompt_ready', summary.get('queued_candidates', 0))}",
            f"- Generated, awaiting approval: {summary.get('generated_awaiting_approval', 0)}",
            f"- Approved but unpublished: {summary.get('approved_unpublished', 0)}",
            f"- Rejected generation attempts: {summary.get('rejected', 0)}",
            f"- Zero-art chapters with no active candidate: {summary.get('zero_art_without_candidate', 0)}",
            f"- Registry live records: {summary.get('registry_live', 0)}",
            "",
            "## Next actionable chapters",
            "",
            f"- Need scene candidates: {_preview(summary.get('top_zero_art_without_candidate', []))}",
            f"- Ready to generate: {_preview(summary.get('top_prompt_ready_chapters', []))}",
            f"- Waiting for image approval: {_preview(summary.get('top_generated_waiting_chapters', []))}",
            "",
            "## Migration status",
            "",
            f"- Unmanaged legacy live art: {summary.get('unmanaged_live_art', 0)}",
