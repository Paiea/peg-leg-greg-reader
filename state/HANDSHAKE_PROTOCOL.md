...
The desired handshake is:

**WORKER A -> writes/validates -> updates GitHub state -> leaves NEXT_TASK -> WORKER B reads GitHub -> continues**
...
When a lane needs an explicit durable trailhead, add/update a compact `NEXT_TASK` section in the lane's existing state/playbook file rather than creating endless handoff files.
...
> Continue Peg-Leg Greg reader work from GitHub. Read project authority plus `state/READER_DESIGN_LAB.md` and visual/production guidance. Use a branch for broad changes. Preserve manuscript prose and newer art/UI work. Validate, update durable design state, and leave the next handshake.

## Visual re-prompt

> Continue Peg-Leg Greg visual production from GitHub. Read `state/VISUAL_BIBLE.md` and `state/IMAGE_PRODUCTION.md`, inspect actual current reader coverage and authoritative manuscript scenes, execute the next coverage batch, integrate only accepted art, validate, update durable state, and leave the next handshake.
...
The worker is responsible for discovering the current state from GitHub.

## Important limitation

Repository state can preserve and route work, but a normal chat does not autonomously launch another chat merely because a commit exists. The practical solution is to make re-prompting tiny and deterministic. Scheduled/agentic environments may automate more of the wake-up step later, but continuity must not depend on that automation.