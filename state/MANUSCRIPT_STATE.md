...
- Do not create new range-stamped running manuscripts for forward work.
- Existing range-stamped files are historical predecessors only.
...
- Preserve canon, characterization, plot, scene intent, and outcomes unless repairing an obvious contradiction.
- NO EM DASHES in manuscript prose.
- Greg POV. Greg is intelligent, observant, practical, and fallible, not a generic mastermind/action hero.
- Theatre remains work, people, acting, stagecraft, failure, recovery, and another turn.
- Long-range theatre → performance → misdirection → illusion → masks → control of perception → eventual dangerous bluff remains directional only.
- Hessa owns mana safety/evidence, not theatrical illusion.
...
- Ch225 the blue-cloth job has been delivered; Lyssa works on a brown sleeve whose customer changes the cuff twice, then returns to a different green garment.
...
- Hard original terms: 35% interest, 8 days, labor through Antonius on default. Original principal unknown.
...
## Bren / pressure arc
- Bren remains probable name of the close-haired, heavy-coat spokesman; left glove, holds that hand closer.
- No surname, boss, organization, or larger structure established.
- Ch218: direct ONE COPPER demand refused by Rinna; Bren says `Next time won't be one.`
- Ch219: thin-faced brown-coat man with dark hair/quiet voice interferes with Olin's oil delivery.
- Ch220: same-description man returns to Olin and asks money-handling questions; Olin gives no useful information. Olin's boy later sees him walking toward Mason's Cut but does NOT see him enter.
...
- Hessa returns after four chapters of breathing room. No power-up, no range mapping, no new distance, no new direction, no theatre/magic connection, and no Bren movement.
- Hessa's practical note schedules Greg after midday in the same room and explicitly tells him to eat and not arrive tired and call it data.
- Greg performs light ordinary theatre work first: holds window-trim boards, sorts cord/pages, has no role, sees the repaired cart working normally, and receives no new threat word from Rinna.
- Same Hessa apparatus is preserved aggressively: room, board, glass shield, paper-fin target/control, chair, INTERMEDIATE mark, FAR mark, body geometry, AWAY only.
- Hessa reveals the withheld question from Ch221: whether the apparent INTERMEDIATE/FAR difference repeats in the same session when order is fixed before testing.
- Predeclared order is FAR → INTERMEDIATE → FAR → INTERMEDIATE so the apparently easier condition does not always receive Greg first/fresh.
- One supervised minimal draw succeeds cleanly, bringing count to 32.
- One supervised deliberate shaping attempt is completed and counted, bringing count to 28. Greg notices his shoulder wanting to rise and corrects before it actually moves; no injury or contamination follows.
- Attempt 38 FAR/AWAY: no observed response; control unchanged.
- Attempt 39 INTERMEDIATE/AWAY: clean small target response with net AWAY displacement and minor rotation; control unchanged.
- Attempt 40 FAR/AWAY: target and control both move. Hessa classifies it as contaminated for target-specific interpretation. Cause is not established. It counts as an external attempt but does not count as clean FAR-response evidence.
- Greg experiences ordinary forearm warmth and right-leg stiffness from sitting. Hessa stops for a break, re-baselines him, and refuses to add/repeat an attempt merely because 40 was contaminated.
- Attempt 41 INTERMEDIATE/AWAY: clean small target response, net AWAY, no obvious rotation; control unchanged.
- Session conclusion stays narrow: the current apparent INTERMEDIATE/FAR difference remains supported under the current apparatus; INTERMEDIATE continues to produce observed response more consistently than FAR; FAR reliability remains unestablished; no range; not generalized.
- Restrictions remain unchanged.
- Hessa has several possible next questions but has not chosen one because the record changed.
- Home: Lyssa correctly understands why attempt 40 is not discarded as useless but also not counted as clean target-specific evidence. Greg tries to turn it into a larger lesson; Lyssa remains sewing.

## Chapter 227 — THE BUNDLER
- Magic rests completely. No Hessa appearance, experiment, home practice, speculative test design, count change, or restriction change.
- Lyssa is already out when Greg wakes. Her green garment and one prior sleeve have moved off the table; a dark skirt is active and she leaves `DON'T WAIT FOR ME FOR SUPPER / EAT THE BREAD.` Her workday is underway independently of Greg.
- With no theatre call, Greg voluntarily checks Vale's office for suitable work rather than waiting for a summons.
- The ink-fingered clerk assigns a different ordinary task from Ch224: sorting and tying outgoing claims/account notices into runner bundles by account/house mark, street, route exceptions, and hold status.
- Greg learns the office's local routing marks through doing. A slashed mark means hold; an unconfirmed new collection account stays rather than being guessed into a route.
- Greg's first bundle knot is poor. The clerk reties it and gives the practical correction `Loop under twice.` Greg improves enough that later bundles are checked for contents rather than knot quality.
- Greg makes one real routing mistake: he sends a South Steps account toward the south runner because he follows the main house/street marks and misses a smaller upper-door triangle. The clerk corrects him; Greg rechecks the bundle and finds two more upper-lane exceptions before dispatch.
- Greg manages ordinary right-leg stiffness from prolonged sitting by standing and walking briefly with his crutches. No injury or medical subplot develops. Office workers make small practiced space/chair adjustments without discussion.
- Greg notices two same-day Fen Pottery packets with the same account/address but conflicting route instructions. The conflict is not criminal or mysterious: a faint black collection dot transferred accidentally from reused paper/ink. Sel and the experienced clerk diagnose and correct it. If missed it would likely have caused an ordinary delay, not catastrophe.
- Antonius receives the clerk's work report, records legitimate debt credit, and keeps the interaction commercial. Exact credit and debt balance remain unstated. Antonius says outgoing bundles happen most days, establishing possible future suitable work when needed, not guaranteed employment.
- No Vale/Bren question or connection appears.
- Greg stops briefly at the theatre on the longer route home. Rinna gives the entire threat update as `No.` No new warning/evidence appears.
- Theatre remains workplace texture only. Rinna and Greg divide a stack of pages when carrying all of it awkwardly fails. Nessa tells Greg he is expected tomorrow afternoon for **The Wrong Funeral**, six lines, role/function `Man with flowers`; Marek has apparently already `borrowed` the flowers. No rehearsal/performance occurs in Ch227.
...
- Ch231 theatre breathes. Greg only returns Nessa's black Magistrate coat in the morning and sees the prior set already repurposed; no rehearsal or performance follows. Rinna says `Tomorrow maybe`, which is not a firm call.
- With no theatre work that day and active debt, Greg voluntarily checks Vale's office for suitable labor. Outgoing bundles are not ready, so the ink-fingered clerk assigns returned-payment acknowledgment matching instead.
- Matching rule is narrow: compare name, house mark, date, and amount against the account line; cross matches, turn disagreements or uncertainty sideways, and the clerk checks Greg's crosses. Local correction/district marks remain contextual rather than universal.
- Greg correctly flags several mismatches and one office carry-forward error involving an earlier partial payment. The office resolves it without mystery or threat significance.
- Greg makes one ordinary mistake by matching the first familiar-looking surname at a shared house instead of checking the full page; two sisters have near-matching names. The clerk catches it before anything leaves the desk.
- Antonius records legitimate debt credit. Greg sees and knows the updated balance, but narration does not state the figure. Debt is smaller, still active, and not small. No guaranteed Vale post, schedule, rate, or promotion is established.
- Body cost remains ordinary seated-work stiffness/eye/back fatigue; new left crutch tip remains normal.
- No Hessa appearance or magic attempt. Counts remain 33 / 29 / 45; all restrictions unchanged. No Bren/pressure evidence or investigation.
...
- Chapter 237 fulfills the concrete Ch236 theatre obligation. Greg rehearses and performs **The Missing Key** as Messenger: eight lines, three entrances, short dark-green coat, three differently folded message props.
- Cast for **The Missing Key** in this chapter: Hara = Mistress, Marek = younger brother, Pell = family Clerk, Greg = Messenger. The cabinet-key farce culminates in the third message saying the key is under a blue vase Marek's character broke before the play began.
- The previously sticking theatre back latch is now repaired and closes cleanly. Davin says the work is mostly his; Jori credits the file. This remains ordinary maintenance, not a threat response.
- Rehearsal keeps disability geometry practical: Greg's easiest entrance line crosses Pell; Jori gives him a clear lane behind Pell and a small route mark. Nessa's short coat and inside pocket keep the three notes clear of the crutches. No injury or new body limitation occurs.
- Greg's theatre-specific correction is about lingering after delivery because he wants to watch the message land. Teren asks why he is still there and narrows the job to delivering the message and exiting because the Messenger has other work.
- Greg also initially jumps at the word `messenger` before his actual second-entrance cue; rehearsal corrects this before performance.
- During performance, Greg repeats the underlying mistake on the second entrance by lingering for his laugh. Hara asks the next written question while he is still present, and Greg accidentally speaks the third-entrance line `No key` too early.
- Greg does not invent a brilliant repair. Hara continues with `Wonderful. We have acquired a messenger who spoils tomorrow.` Later, at the real third entrance, Pell keeps the scene moving by taking the note and saying they should read it. The play survives.
- Hara owns the strongest audience beat after the third message identifies the blue vase: she turns on Marek's character and asks `Which piece?` Marek's denial/blue-vase business carries the larger laugh.
- Teren's post-show note stays narrow: Greg stayed for the laugh; if the laugh is his, he should leave with it rather than remain to watch it. This is theatre-specific and not converted into a life maxim.
- **The Missing Key** remains conditional for tomorrow: Teren says `Again tomorrow if it stays paired.` Greg remains Messenger unless Teren changes it. This is not a guaranteed performance or permanent role.
- Rinna pays the normal one copper. No Vale appearance/work/credit occurs; debt remains active and unstated.
...
- Chapter 242 lets Ch241's scenery-labor day breathe. No theatre call/work/pay, Hessa appearance/magic attempt, Bren/brown-coat fact, warning, investigation, or pressure escalation occurs. Magic remains **35 / 31 / 53** with all restrictions unchanged.
- Ordinary equipment wear creates the day's first real claim: the right crutch grip wrapping has shifted around the handle, putting its seam against the base of Greg's thumb. There is no tear, exposed wood, sharp tack, pain, weakness, or new medical limitation.
- Greg returns to the same established crutch repairer from Ch223, who already knows his equipment and has previously handled grips/ferrules. The repairer judges the existing wrap still good, loosens only part, retensions it evenly, and adds one small flush tack beneath the handle overlap away from Greg's palm. He checks the left grip and explicitly leaves it alone because it is fine.
- Greg tests both turn directions in the shop. The repaired right grip stays flat/even and the tack does not contact his hand. By the end of the day Greg has stopped noticing the repair. The repaired LEFT tip remains normal.
- Greg pays the ordinary repair cost. The expense makes the active Vale debt salient, so he voluntarily checks the office for suitable work. The ink-fingered clerk has work in the office but **none suitable for Greg that day**. Outgoing packets, acknowledgments, and current sealing are already done. Greg receives **no Vale work and no debt credit**.
- The clerk says sealing may exist `Tomorrow, maybe. Not today.` This is conditional only, not a schedule, promise, guaranteed post, or Chapter 243 obligation.
- Greg buys and eats ordinary stew despite the repair expense and active debt. The one copper from Ch241 was ordinary money and is not automatically converted into debt payment. Debt remains active, exact balance known in-world but unstated, and no Vale/Bren connection is established.
...
- A fresh Rinna call creates a genuinely new theatre obligation rather than continuing Ch243's onion-stall episode or reviving Ch242's Vale maybe. Greg is asked to come before midday if free; Teren gives him the eleven-line, one-entrance/one-exit, mostly seated **Uncle** in a new comedy, **The Guest Pot**.
- Cast established in this chapter: Hara = woman of the house/host, Marek = her younger brother, Venn = household cook, Greg = visiting Uncle. The comic engine is a promised supper that does not exist: Marek's character has spent the food money, Venn knows there is no meal, and the covered serving dish is empty while Greg's Uncle initially expects to be fed.
- Greg's narrow rehearsal problem is visible foreknowledge. Because Greg has read the pages and knows the covered dish is empty, he keeps checking it before the Uncle has any reason to suspect the supper. Teren narrows the playable correction to the character arriving for supper and expecting supper; keep this theatre-specific rather than turning it into a life, threat, magic, or bluff doctrine.
- During performance Greg repeats the mistake once in smaller form by glancing at the covered lid before Venn's Cook reveals that the dish is empty. Some of the audience gets slightly ahead of the reveal and the following laugh is smaller. Greg does not invent a rescue. The scene continues through the company.
- Hara owns the strongest final audience beat: after the empty supper has been exposed and Marek's character offers to fetch food, Hara asks `With what money?`; he sits, and the audience hits hard. Venn remains an ordinary competent company member and says only that he has been at the theatre `Before you` when Greg asks how long.
- Jori/Nessa/Teren keep the work practical. Greg's chair is angled and then shifted/marked for clean crutch geometry without an accommodation speech. Props stay ordinary; the empty covered dish is not a mystery or special object.
- Rinna pays Greg the normal one copper for the performance day. Entering Ch245, **The Guest Pot** is only conditional: Rinna says it is `Not paired yet`; Uncle / same piece are both `Maybe`. No standing theatre obligation is created.
- No Hessa appearance or magic attempt, no Vale appearance/work/credit, and no Bren/brown-coat fact or pressure escalation occurs. Magic remains **35 / 31 / 53** with all restrictions unchanged; debt remains active/unstated; Ch236 remains the newest pressure fact and no Vale/Bren connection exists.
- Body cost is ordinary only: normal right palm, used but fine shoulders, mildly tired right leg from standing between scenes, comfortable residual limb, normal repaired left tip, and both grips normal. No injury or new limitation.
- Home remains bounded. Lyssa is already there with no customer or new dark cloth. Brown cloth remains same fold/three pins; damaged shirt remains three loose threads; better shirt retains the blue streak; onions remain ordinary household food. Beans and the older wrapped item remain absent/unexplained.

## Immediate next edge — Chapter 245
Primary recommendation: **LET CH244'S FULL THEATRE REHEARSAL/PERFORMANCE DAY BREATHE. FOLLOW THE NEXT ACTUAL CLAIM RATHER THAN MAKING RINNA'S `MAYBE`, THE EMPTY DISH CORRECTION, OR CH240'S FAR RESPONSE INTO AN AUTOMATIC NEXT STEP.**

Reasons:
- Ch240 was a full bounded Hessa FAR-only session; Ch241 was ordinary theatre scenery labor; Ch242 was crutch maintenance plus a Vale check that produced no suitable work; Ch243 was ordinary Carrow market/social life; Ch244 is now a full new-play rehearsal/performance day.
- **The Guest Pot** may recur, but Rinna explicitly says it is `Not paired yet`; Uncle and the same piece are both only `Maybe`. Greg has no standing Ch245 theatre obligation.
- Greg's Ch244 mistake is narrow acting craft: he visually telegraphs the empty serving dish because he knows the script before the Uncle does. Do not convert that into a universal rule about deception, threat response, magic, or the long-range bluff. It can remain one theatre correction until later accumulation genuinely earns more.
...
- Vale debt remains active/unstated. Ch242's `Tomorrow, maybe` already expired without creating work in Ch243 and remains nonbinding. Ch244 contains no Vale movement.
- External pressure still last moved in Ch236. Ch237–244 add no new clue or defensive escalation. The repaired theatre back latch remains ordinary maintenance and no Vale/Bren connection exists.
- Ch243's onion-seller social memory is now established but does not create employment, a recurring stall job, or a need for another market chapter.
...
Strong Chapter 245 possibilities:
- ordinary Carrow or an established social route with its own momentum;
- Lyssa/household/work only if a real obligation surfaces without servicing tracked objects by checklist;
- Vale only if suitable ordinary work actually exists through a fresh circumstance, not because of the stale Ch242 maybe;
...
- theatre only if the conditional pairing becomes an actual fresh call. Do not assume **The Guest Pot** repeats.

Ask: **WHAT ACTUALLY HAS A REASON TO HAPPEN TODAY?** Favor **LIFE PER WORD**.

Preserve magic counts **35 / 31 / 53** and all restrictions. Preserve Attempt 52 as one clean FAR/AWAY response without power-up. Preserve active unstated Vale debt, Ch238 credit, Ch242's no-work/no-credit result, and the fact that its `Tomorrow, maybe` did not create later work. Preserve Ch236's delivery/access evidence ceiling and no Vale/Bren connection. Preserve ordinary back-latch interpretation, Greg's body continuity, successful right-grip repair and normal left tip/grip, Ch243 onion-seller social memory without employment, Ch244's narrow empty-dish acting correction without doctrine, conditional-only **Guest Pot** status, brown-cloth identity limits, three-thread damaged-shirt continuity, blue-streak better-shirt residue, bean-purpose limits, absent older wrapped-item identity ceiling, absent Ch241 dark-cloth identity ceiling, Lyssa customer/garment boundaries, and **Marra ≠ Maren**.

## Chat / handoff behavior
After each completed chapter:
1. update permanent manuscript;
2. update this state in place;
3. update index/open threads only with durable residue;
4. update workflow/project endpoint;
5. preserve reader UI/artwork and known 156–219 synchronization gap;
6. compact production note in chat;
7. full copyable next-edge re-prompt in one code block.

GitHub is durable memory. The re-prompt is the steering wheel.