# MANUSCRIPT OPEN THREADS

## Current edge
- Current exact story endpoint: Chapter 303 — **THE BLUE HORSE**.
- Exact Chapters 249–300 are durable checkpoint files and outrank summaries.
- Permanent running manuscript remains physically materialized through Chapter 251; exact-text integration debt remains.
- Canonical inserted Chapters **220A — THE SHORTAGE**, **220B — THE OLD MAN**, and **220C — THE ROUTE** sit between Chapters 220 and 221 and do not renumber later chapters.
- See `MANUSCRIPT_STATE.md` for executable Chapter 304 trailhead.

## Direction
- Preserve ordinary lived detail, social accumulation, disability/material reality, humor, work, and independent side-character causality.
- Magical ecology/fieldcraft remains a strong push without creature-of-the-week, but not every chapter must service it.
- Ch301 personal magic remains background evidence rather than triggering an automatic second session.
- Ch302 basket/Olan/Mera sequence stands complete.
- Ch303 follows a one-off household caregiving claim and embeds ordinary child-scale magic through a neighborhood kite court, not through a new repair/investigation lane.
- Greg's world familiarity remains calibrated: broad lift-bead and weak-deflection families are familiar; Vara/Ren hold local practical knowledge.
- Chapter 303 is not a structural milestone. Scope restraint remains behavioral.

## Magic / Hessa / range protocol
- Totals remain **36 supervised minimal draws / 32 deliberate shaping attempts / 72 supervised external-effect attempts**.
- No independent magic outside supervised sessions Ch284–300. Restrictions unchanged.
- Ch301 attempt 70 near: small clean net displacement away.
- Ch301 attempt 71 middle: no observed response.
- Ch301 attempt 72 far: first far test, no observed response.
- Established-position aggregates remain **near 2 clean / 4; middle 1 clean / 6 plus one prior possible edge lift (attempt 68); far 0 / 1**.
- No distance-response law, reliability curve, hard range limit, or dose relationship established.
- Hessa next experiment, position, object, and appointment remain unselected.
- Ch303 lift bead/drift web are external ordinary magic and do not alter Greg totals.

## Tavi / bench court / Vara / Ren / blue horse
- Ch303 Cala gives Greg one bounded task: take restless Tavi to bench court, not market, and sit by the gate.
- Route rules from Cala: `wall side` and `stop corners`. Tavi follows them adequately and stops unprompted at later corners.
- Greg cannot normally hold Tavi's hand while walking with two crutches; bench-court geometry lets supervision depend on quiet lanes, corner stops, and a gate position rather than chasing.
- Bench court: two quiet lanes away; old-brick walls on three sides, low iron gate on fourth, wall benches, central rain tree, packed earth, ordinary neighborhood children/play.
- Vara is an older woman shelling peas on the gate bench. She knows Cala and recognizes Greg; Greg learns her name only in Ch303. Exact prior familiarity/formal role unknown.
- Vara notices Greg's right crutch grip projecting into the walking path and tells him to `Sit better`; he adjusts it.
- Ren is a boy Greg estimates around ten or eleven who is flying the blue horse kite. No standing relationship established.
- Tavi is allowed to hold a separate plain red tail loop, not the main kite line.
- During a snag, the spool rolls toward the gate. Greg tells Tavi `Stay`, pins the loose line briefly with his right crutch tip, and releases once Ren recovers the spool. No injury/equipment damage.
- Kite catches low in rain tree. Greg does not use his crutch to retrieve it. An unnamed girl supplies a forked branch and helps Ren free it.
- Tavi catches the falling tail/cloth against his chest; cloth remains fine.
- Vara inspects and says `Bead fine. Knot slipped.` She recenters bead and reties local stop knot with two turns for today's west wind rather than yesterday's one turn.
- Tavi later draws a blue chalk `horse` on the court wall. No household paper consumed.
- Return-home comic residue: Tavi tells `Greg pole` / `Greg horse tree`; Greg denies responsibility. Cala's practical check is whether Tavi ran; he did not leave the court.
- Do not auto-create a regular Greg/Tavi outing, Vara mentorship, Ren friendship, kite purchase, kite repair lane, drift-web investigation, or next-day return.

## Blue horse kite / lift bead / drift web
- Blue horse kite: badly painted cloth horse, roughly four feet long by Greg's visual estimate; small old brass lift bead beneath first cross spar.
- Lift bead flashes pale gold when nose dips and helps cloth recover air. Greg recognizes broad family from survey/messenger/camp-signal kites; exact mechanism, maker, charge, lifespan, and safe envelope unknown.
- Four short roof-corner posts support faint amber drift web above court. When kite nears edge, web briefly appears honey-colored and nudges it inward.
- Exact installation, owner, maintenance, power source, force, height, limits, and whether it is designed exactly as Greg infers remain unestablished.
- Vara's `two turns today / yesterday one` is local kite practice tied to west wind in this scene, not universal rule.

## Mera basket / Olan carry tie
- Ch302 facts stand complete. Ch303 begins after Mera has collected Lyssa's cloth and does not reopen basket durability, carry tie, Olan, or courier work.
- Exact maker, age, charge/lifespan, easing mechanism, safe max load, broader standards, and repair history remain unknown.

## Lyssa / garment work
- Lyssa independent garment-work network remains active.
- Ch302 Mera delivery completes off the front of Ch303.
- Ch303 Lyssa returns upstairs to brown-cuff work and later comes down with the cuff folded over one arm.
- No customer identity, deadline, price, alteration request, completion, delivery, or queue consequence is established in Ch303.
- Greg does not join, optimize, analyze, or manage the work.
- Existing protected Lyssa materials/packets/bundles remain protected.

## Money / capital
- Greg holds **three private client copper** upstairs.
- Ch303 no income and no spending.
- No household money carried/spent.
- No copper/bit conversion established.

## Greg carry bag / body / equipment
- Ch296 brown leather shoulder bag remains regular equipment; stay-bead notch unchanged.
- Ch303 bag carries only existing charcoal/narrow scrap and returns to peg normally.
- Greg is nineteen, Bronze, Carrow, permanent LEFT BKA, knee preserved; right leg intact; two crutches.
- Ch303 outing/body: no pain, fall, pressure injury, residual-limb issue, crutch damage, shoe issue, bag issue, or equipment change.
- Bench sit/rise is ordinary with right foot/bench support and left knee clear.
- Brief line-pin maneuver uses right crutch tip only; no damage or loss of balance.

## High line / Mara / Ovi / Tess
- Ch300 facts stand. Ch301–300 do not return.
- Tess remains only expected noon repair person, unseen. Repair outcome/mechanism/rate unknown.
- Do not auto-return or make high-line work.

## Kessa / Pella / Rusk / silverbacks-flickers
- Ch300 facts stand. Ch301 has one passing recognition; Ch302–300 no follow-up.
- Exact taxonomy, naming boundary, lifecycle, prey range, `skins`, magical status, thresholds, and cool/sun behavior remain unresolved.

## Nemi / workshop-court music / road song
- Ch299 facts stand. Ch300–300 no substantive follow-up.
- No music group, lesson, performance job, songwriting lane, theatre crossover, or hold-bead investigation.

## Bridges / Vela / Orra / Demin / Sot
- Ch298 facts stand. Ch299–300 no return.
- Vela's `Tomorrow?` remains casual and non-binding.

## Household shopping / merchant memory
- Ch297 facts stand. Ch298–300 no meaningful shopping follow-up.
- No formal household account/credit/discount/network.

## Shade fish / east-wall trough / water list
- Ch295 facts stand. Ch296–300 do not return.
- Weak sun-turn bead remains only marked/managed as of last sighting; fish fine; replacement schedule unknown.

## Door closer / Olan / Sella
- Ch294 front door repair stands and functions cleanly in Ch302–300.
- Olan's Ch302 basket appearance does not establish Greg apprenticeship, standing work, or general Olan lane.

## Rain shelf / rain thread
- Ch293 facts stand. Household rain shelf works. Ch294–300 no reactivation.

## Miri / cooling cups
- Ch291 facts stand. Ch292–300 no follow-up.
- Miri one-copper comparison and possible Pellin follow-up remain bounded/unresolved. Pellin is not Pell absent evidence.

## Carrying light
- Ch290 facts stand. Ch291–300 do not advance Sera or lantern house.

## Ward crossing
- Ch292 repair stands. Ch293 passes it in rain. Ch294–300 no advancement.
- Ch295 water worker/list remains separate from road crew/list.

## Dry seal / warm wall
- Ch289 old dry seal stands as resolved ordinary household maintenance unless independently reactivated.

## Magical ecology / chaffers
- Ch287 chaffer hatch stands, mostly over.
- Ch295 shade fish remain separate urban ecology.
- Ch300 kessa jumpers remain separate local ecological practice.
- Ch301–300 do not advance ecology as a work lane.
- Ch303 play-court magic is material/civic play infrastructure, not an ecology case.
- Do not build creature-of-week or ecology-ledger structure.

## Theatre / Pavin
- Ch279 one-house Masked Guest replacement and Ch285 paid Pavin catch-up rehearsal stand.
- Ch286–300 no theatre contact. No promotion/standing role.

## Rell / Maren / return hardware
- Ch284 bounded return-hardware identification stands. Hardware unactivated/unvalued.
- Exact maker/mechanism/pairing/activation/safety/value unknown. Jori only possible appraisal route.
- Rell referral proposal unresolved.

## Household
- Three clean household gray sheets remain.
- Horse-page reverse remains full.
- Three private client copper remain upstairs.
- Existing narrow scrap remains in Greg's bag; no Ch303 notes added.
- Tavi's blue drawing is public wall chalk, not a new household page.
- Rain shelf works. Front door closer repaired/functioning.
- Ch288 narrow wrapped packet remains absent/protected.

## Economy / debt
- Currency copper/silver/gold; no conversion rate. Bits small-price unit; conversion unstated.
- Vale debt active/unresolved; exact balance unstated.

## Bren / local extortion repair
- Canonical inserted sequence is **220A THE SHORTAGE → 220B THE OLD MAN → 220C THE ROUTE**, between Ch220 and Ch224. Later chapter numbers do not shift.
- Ch220 establishes Olin delivery questioning and Rinna's question-sharing response.
- Ch221 makes the threat materially expensive: Olin stops sending his boy while two men watch/question the route, and Nessa/Jori must collect oil themselves.
- Ch222 fulfills Bren's `Next time won't be one` threat. Bren returns with two men and demands five copper. Rinna remains the theatre authority. Greg uses stagecraft, prepared blue-glass theatrical magic, old-life procedural threat-reading, and deliberate performance to make certainty expensive. Greg casts no new personal illusion spell and gains no new independent magic capability.
- Bren recognizes that some effects are theatre but cannot cleanly separate staged effects from Greg's genuine knowledge/history. His group withdraws. The win is uncertainty/delay, not domination.
- Ch222 also demonstrates the clarified memory pattern: deeply practiced procedural recall can survive while an attached autobiographical identifier, such as a teacher's name, does not.
- Ch223 makes the quiet resolution legible: Rinna's informal business network shares questions, reduces useful information, adds witnesses/friction, and Olin's ordinary delivery eventually resumes.
- Treat the **local theatre extortion pressure as resolved/dormant** unless later causality independently reactivates it. Do not auto-return Bren.
- Bren surname, employer/boss, organization, full network scale, permanent whereabouts, and whether every watcher worked for him remain unresolved.
- **Vale/Bren link remains unresolved and unestablished.**
- See `state/BREN_220ABC_CANON.md` for the dedicated insertion state.

## Pressure / protected uncertainty
- Ch276 Green Coat remains separate/bounded. Ch303 adds no new pressure evidence.
- The older Bren/theatre pressure is locally resolved through Ch221–C as above; broader Bren criminal ecology remains protected uncertainty rather than an active automatic threat lane.
- Weak needle-case mark, Rinna parcel, Lyssa older materials/bundles, Ch249 crate contents, Pell exact return, Vale/Bren link unresolved.
- Collar-facing broader purpose, six wraps, Ch284 flat package, Ch288 narrow wrapped packet unresolved.
- Magic protected uncertainty remains exact range distances, far 0/1 only, attempt 68 possible edge lift only, next protocol unselected, no range law.
- Mera basket/carry-tie uncertainties remain bounded.
- Bench-court/kite/drift-web uncertainties remain bounded as listed above.
- Existing creature/ecology and magical-infrastructure uncertainties remain bounded.

## Next
- Follow Chapter 304 trailhead in `MANUSCRIPT_STATE.md`.
- Let Ch303 Tavi/bench-court/Vara/Ren/blue-horse sequence stand. No automatic return, regular babysitting lane, kite purchase, repair, ward investigation, or Vara mentorship.
- Let Ch302 basket sequence and Ch301 far test stand.
- Current Greg magic remains 36 / 32 / 72; near 2/4 clean; middle 1/6 clean + one possible edge lift; far 0/1. Restrictions unchanged.
- Three private client copper and three clean sheets remain.
- Chapter 303's round number created no structural event; do not create a delayed one merely because 301 follows it.
- Follow the next actual claim rather than rotating available engines by checklist.
