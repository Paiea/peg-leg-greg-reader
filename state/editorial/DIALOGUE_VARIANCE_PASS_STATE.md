...
- Chapter 1 `THE BOY`: no variance change required.
- Chapter 2 `THE BORROWER`: no new variance change required. Antonius's clipping is operational loan-pricing behavior; Sella refuses Greg's framing rather than matching it.
- Chapter 3 `THE INVESTOR`: **variance patch required**. Arlo's technical identity is real, but several exchanges give him Greg-compatible polished counterpunch timing. Decompress selected replies into workshop capacity, labor cost, and next-test decisions.
- Chapter 4 `THE EXPERT`: no variance change required. Table dialogue and Senna's refusal are poker/social-pressure shaped rather than generic Greg voice.
- Chapter 5 `THE WARRIOR`: no variance change required. Jorren's short lines are embodied and physical; Antonius's repeated naming critique is ledger-specific and tied to Greg's actual borrower behavior.

## Chapters 6-10 result

- Chapter 6 `THE LABORER`: **light variance patch required**. Rusk remains strongly operational, but one opening counter and Hessa's post-warehouse aphoristic punchline are better converted into task/state control.
- Chapter 7 `THE OWNER`: **3 variance patches required**. Antonius is over-optimized in several broom/storage exchanges. Shift selected replies toward moving the work, storage consequence, and buyer reality.
...
## Chapters 26-30 result

- Chapter 26 `THE PATIENT`: **moderate variance patch required**. Doven/Nemi operational command remains strong, but the first healer repeatedly slips into generic Greg-management comedy. Shift selected lines toward treatment sequence, restriction duration, and medical compliance.
- Chapter 27 `THE RESCUER`: **no variance change required**. Iris's shortness is rescue procedure; Senn, Eda, Mira, and Jory provide fear, age, repetition, local grievance, and imperfect emergency behavior.
...
Do not:
- merge chapters;
- delete scenes merely for pace;
- renumber;
- alter canon outcomes;
