...
class BookVStructureTests(unittest.TestCase):
    ...
    def test_book_v_uses_uploaded_investor_role_card(self):
        book_five = BOOKS[4]
    ...
    def test_renderer_marks_book_v_as_current_at_chapter_480(self):
        links = {n: f'<a href="chapters/{n:03d}.html">Chapter {n}</a>' for n in range(1, 481)}
        rendered = render_book_sections(links, illustrated=True)
        self.assertIn('BOOK V', rendered)
        self.assertIn('Chapters 441–480', rendered)
        self.assertIn('ACT I · Chapters 441–480', rendered)
        self.assertIn('THE LONGER REACH', rendered)
        self.assertIn('book-v-investor-446-q80.webp', rendered)
        self.assertIn('id="book-v-heading" aria-current="true"', rendered)
        self.assertNotIn('Chapters 321–480', rendered)
