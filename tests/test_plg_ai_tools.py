...
class PLGAIToolTests(unittest.TestCase):
    ...
    def test_query_scenes_delegates_to_project_index(self):
        ...
        self.assertEqual([{"scene_id": "214.s020"}], result)
    ...
    def test_plan_and_run_delegate_to_campaign_runner(self):
        with mock.patch.object(plg_ai_tools.performance_campaign, "plan_campaign", return_value={"campaign_id": "x"}) as plan:
            self.assertEqual({"campaign_id": "x"}, plg_ai_tools.plan_campaign({"task": "reverse_edit", "chapter_start": 1, "chapter_end": 2, "source_authority": "sha"}))
            plan.assert_called_once()
        with mock.patch.object(plg_ai_tools.performance_campaign, "run_campaign", return_value={"completed": 2}) as run:
            self.assertEqual({"completed": 2}, plg_ai_tools.run_campaign({"campaign_root": "/tmp/campaign"}))
    ...
    def test_run_campaign_can_plan_and_execute_from_one_ai_tool_call(self):
        ...
        self.assertEqual("all", result["campaign_id"])
        self.assertEqual(300, result["plan"]["packets_planned"])
        self.assertEqual(300, result["run"]["completed"])
    ...
    def test_read_scene_view_does_not_mutate_source_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("001.s010", result["scene_id"])
    ...
    def test_tool_metadata_distinguishes_read_only_from_disposable_cache_writes(self):
        self.assertTrue(plg_ai_tools.TOOL_SPECS["get_scene_view"]["read_only"])
        self.assertTrue(plg_ai_tools.TOOL_SPECS["query_scenes"]["read_only"])
        self.assertTrue(plg_ai_tools.TOOL_SPECS["get_campaign_result"]["read_only"])
    ...
    def test_call_dispatches_structured_payload(self):
        ...
        with self.assertRaisesRegex(ValueError, "unknown PLG AI tool"):
            plg_ai_tools.call_tool("nope", {})
